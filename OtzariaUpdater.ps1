...
function Apply-Theme {
    ...
    foreach ($btn in @($btnDL, $btnCancel, $btnPreChangelog, $btnLibDL, $btnLibCancel, $btnLibChangelog, $btnLibExtract, $btnLibSelectFile, $btnClearCache, $btnVCRedist, $btnTheme, $btnRefresh, $btnSelectFile)) {
...
$btnRefresh.Add_Click({
    ...
    $libVerJob = Start-Job -ScriptBlock {
        try {
            $r = Invoke-RestMethod "https://api.github.com/repos/Y-PLONI/otzaria-library/releases/latest" -Headers @{"User-Agent"="PS"} -TimeoutSec 15
            if ($r.tag_name -match '(\d+)$') { return $matches[1] }
        } catch { }
        return "?"
    }
