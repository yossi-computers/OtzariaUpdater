...
$form.Add_Shown({
    ...
    if (-not $existingLibZip) {
        ...
        $existingLibZip = Get-ChildItem "." -Filter "otzaria_*.zip" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    }
    if ($existingLibZip) {
        ...
        if ($newerAvailable) {
            ...
            # Total width ~450, buttons 130 each with 30 spacing, centered in 654 card
            # Start at (654-450)/2 = 102
            $btnLibDL.Text = "הורדה"
            $btnLibDL.Size = New-Object System.Drawing.Size(130,34)
            $btnLibDL.Location = New-Object System.Drawing.Point(102,72)
            $btnLibDL.Visible = $true
            
            $btnLibUpdate.Text = "עדכון ארכיון"
            $btnLibUpdate.Size = New-Object System.Drawing.Size(130,34)
            $btnLibUpdate.Location = New-Object System.Drawing.Point(262,72)
            $btnLibUpdate.Visible = $true
            
            $btnLibExtract.Text = "חלץ לאוצריא"
            $btnLibExtract.Size = New-Object System.Drawing.Size(130,34)
            $btnLibExtract.Location = New-Object System.Drawing.Point(422,72)
            $btnLibExtract.Visible = $true
...
$btnRefresh.Add_Click({
    ...
    $stableJob = Start-Job -ScriptBlock {
        try {
            $r = Invoke-RestMethod "https://api.github.com/repos/Otzaria/otzaria/releases" -Headers @{"User-Agent"="PS"} -TimeoutSec 15
            $rel = $r | Where-Object { $_.prerelease -eq $false } | Select-Object -First 1
            if (-not $rel) { return @{Release=$null; Changelog=""} }
            $a = $rel.assets | Where-Object { $_.name -like "otzaria-*-windows.exe" -and $_.name -notlike "*-full.exe" } | Select-Object -First 1
            if (-not $a) { return @{Release=$null; Changelog=""} }
            $changelog = if ($rel.body) { $rel.body } else { "אין מידע על שינויים" }
    ...
    $preJob = Start-Job -ScriptBlock {
        try {
            $r = Invoke-RestMethod "https://api.github.com/repos/Otzaria/otzaria/releases" -Headers @{"User-Agent"="PS"} -TimeoutSec 15
            $rel = $r | Where-Object { $_.prerelease -eq $true } | Select-Object -First 1
            if (-not $rel) { return @{Release=$null; Changelog=""} }
            $a = $rel.assets | Where-Object { $_.name -like "otzaria-*-windows.exe" -and $_.name -notlike "*-full.exe" } | Select-Object -First 1
            if (-not $a) { return @{Release=$null; Changelog=""} }
            $changelog = if ($rel.body) { $rel.body } else { "אין מידע על שינויים" }
