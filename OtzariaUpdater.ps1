...
$btnDL.Add_Click({
    ...
    $oldFiles = @()
    if ($script:InstallType -eq "MSIX") {
        $oldFiles = Get-ChildItem "." -Filter "*.msix" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne $script:FinalFile }
    } else {
        # For EXE, check both regular and full versions
        $allExeFiles = Get-ChildItem "." -Filter "otzaria-*-windows*.exe" -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne $script:FinalFile }
        $oldFiles = $allExeFiles
    }
    
    if ($oldFiles -and $oldFiles.Count -gt 0) {
        $fileList = ($oldFiles | ForEach-Object { $_.Name }) -join "`n"
        $r = [System.Windows.Forms.MessageBox]::Show(
            ":נמצאו גרסאות ישנות בתיקייה`n$fileList`n`n?האם למחוק אותן",
            "מחיקת גרסאות ישנות",
            "YesNo",
            "Question"
        )
