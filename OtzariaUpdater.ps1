...
$btnLibExtract.Location = New-Object System.Drawing.Point(267,72)
$btnLibExtract.Size = New-Object System.Drawing.Size(120,34)
...
$btnLibDL.Location = New-Object System.Drawing.Point(247,72)
$btnLibDL.Size = New-Object System.Drawing.Size(160,34)
...
$btnTheme.Text = "מצב כהה"
$btnTheme.Size = New-Object System.Drawing.Size(110,40)
$btnTheme.Location = New-Object System.Drawing.Point(25,685)
$btnTheme.FlatStyle = "Flat"
$btnTheme.Font = New-Object System.Drawing.Font("Segoe UI",10)

$statusDivider = New-Object System.Windows.Forms.Label
$statusDivider.Location = New-Object System.Drawing.Point(15,740)
...
$statusLocation.Location = New-Object System.Drawing.Point(15,748)
...
$statusBar.Location = New-Object System.Drawing.Point(15,768)
...
$lblNetwork.Location = New-Object System.Drawing.Point(500,795)
...
$form.Controls.AddRange(@($header,$mainCard,$libCard,$btnClearCache,$btnTheme,$statusDivider,$statusLocation,$statusBar,$lblNetwork))
...
function Check-DownloadedFiles {
    ...
    if ($script:InstallType -eq "MSIX") {
        ...
        if ($existingMsix -and $existingMsix.Name -match 'otzaria-([\d\.]+)\.msix$') {
            $fileVer = $matches[1]
            # Check if this version is different from server versions
            ...
            if ($fileVer -ne $stableVer -and $fileVer -ne $preVer) {
                $script:DownloadedRelease = @{ FullVersion = $fileVer; File = $existingMsix.Name; Url = $null }
        ...
        # Prefer full EXE, then regular EXE
        if ($existingFullExe -and $existingFullExe.Name -match 'otzaria-([\d\.]+)-windows-full\.exe') {
            $fileVer = $matches[1]
            if ($fileVer -ne $stableVer -and $fileVer -ne $preVer -and $fileVer -ne $fullVer) {
                $script:DownloadedRelease = @{ FullVersion = $fileVer; File = $existingFullExe.Name; Url = $null }
                $radioDownloaded.Visible = $true
                $lblDownloadedV.Text = $fileVer
                $lblDownloadedV.Visible = $true
            }
        } elseif ($existingExe -and $existingExe.Name -match 'otzaria-([\d\.]+)-windows\.exe') {
            $fileVer = $matches[1]
            if ($fileVer -ne $stableVer -and $fileVer -ne $preVer -and $fileVer -ne $fullVer) {
                $script:DownloadedRelease = @{ FullVersion = $fileVer; File = $existingExe.Name; Url = $null }
                $radioDownloaded.Visible = $true
                $lblDownloadedV.Text = $fileVer
                $lblDownloadedV.Visible = $true
            }
...
$form.Add_Shown({
    ...
    if ($existingLibZip) {
        ...
        if ($newerAvailable) {
            ...
            $btnLibDL.Location = New-Object System.Drawing.Point(202,72)
            $btnLibDL.Visible = $true
            $btnLibExtract.Location = New-Object System.Drawing.Point(332,72)
...
$btnTheme.Add_Click({
    ...
    $btnTheme.Text = if ($script:DarkMode) { "מצב בהיר" } else { "מצב כהה" }
    Apply-Theme
})
                ...
                if ($script:SilentInstall) {
                    ...
                    try {
                        ...
                        if ($proc.ExitCode -eq 0) {
                            ...
                            [System.Windows.Forms.MessageBox]::Show("אוצריא הותקן בהצלחה!", "התקנה הושלמה", "OK", "Information")
                        } else {
...
# Cancel Button
$btnCancel.Add_Click({
    $r = [System.Windows.Forms.MessageBox]::Show("?האם לבטל את ההורדה`n.כל מה שהורד עד כה יימחק", "אישור ביטול", "YesNo", "Question")
    if ($r -eq "Yes") {
        "stop" | Out-File "$env:TEMP\otzaria_stop.flag" -Force
        Start-Sleep -Milliseconds 500
        if ($script:DownloadJob) {
...
$btnLibDL.Add_Click({
    ...
    if ($script:LibDownloadCompleted) {
        ...
        $r = [System.Windows.Forms.MessageBox]::Show(
            ".שים לב: כעת יימחקו כל הספרים שבספרייה`n`n?האם להמשיך",
            "אישור מחיקה",
            "YesNo",
            "Warning"
        )
        ...
        try {
            ...
            if (Test-Path $fileToExtract) {
                $delResult = [System.Windows.Forms.MessageBox]::Show(
                    "?האם ברצונך למחוק את ארכיון הספרים",
                    "מחיקת ארכיון",
                    "YesNo",
                    "Question"
                )
...
$btnLibExtract.Add_Click({
    ...
    try {
        ...
        $delR = [System.Windows.Forms.MessageBox]::Show(
            "?האם למחוק את קובץ הארכיון",
            "מחיקת קובץ",
            "YesNo",
            "Question"
        )
...
$btnClearCache.Add_Click({
    $r = [System.Windows.Forms.MessageBox]::Show(
        "?האם אתה בטוח שברצונך למחוק את המטמון של תוכנת אוצריא`n`n!שים לב: כל ההיסטוריה, המועדפים וההעדפות יימחקו ולא יהיה ניתן לשחזרם",
        "מחיקת מטמון",
        "YesNo",
        "Warning"
    )
    if ($r -eq "Yes") {
        ...
        if (Test-Path $cachePath) {
            try {
                ...
                [System.Windows.Forms.MessageBox]::Show("!המטמון נמחק בהצלחה", "הצלחה", "OK", "Information")
            } catch {
                [System.Windows.Forms.MessageBox]::Show("שגיאה במחיקת המטמון: $_", "שגיאה", "OK", "Error")
            }
        } else {
            [System.Windows.Forms.MessageBox]::Show("תיקיית המטמון לא נמצאה", "מידע", "OK", "Information")
        }
    }
})
